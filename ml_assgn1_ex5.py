...
x_3 = X[:, 0]*X[:, 1]
x_3 = np.reshape(x_3, (X.shape[0], 1))  # convert from list to matrix
x_4 = X[:, 0]**2
x_4 = np.reshape(x_4, (X.shape[0], 1))
x_5 = X[:, 1]**2
x_5 = np.reshape(x_5, (X.shape[0], 1))
x_6 = X[:, 0]**3
x_6 = np.reshape(x_6, (X.shape[0], 1))
x_7 = X[:, 1]**3
x_7 = np.reshape(x_7, (X.shape[0], 1))

X = np.append(X, x_3, axis=1)
X = np.append(X, x_4, axis=1)
X = np.append(X, x_5, axis=1)
X = np.append(X, x_6, axis=1)
X = np.append(X, x_7, axis=1)
# /
...
#########################################
# Initialise trainable parameters theta to size 8 since have three new non-linear sets added
theta = np.zeros((8))
# /
...
theta_final, cost_vector_train, cost_vector_test = gradient_descent_training(
    X_train_normalized, y_train, X_test_normalized, y_test, theta, alpha, iterations)
...
print('Minimum train cost: {:.5f}, on iteration #{}'.format(
    min_train_cost, argmin_train_cost+1))
print('Final test cost: {:.5f}'.format(cost_vector_test[-1]))
print('Minimum test cost: {:.5f}, on iteration #{}'.format(
    min_test_cost, argmin_test_cost+1))
